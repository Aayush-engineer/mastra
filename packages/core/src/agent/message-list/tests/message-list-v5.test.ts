...
describe('MessageList V5 Support', () => {
  ...
  describe('toModelOutput support', () => {
    ...
    it('translates tool-result media to tagged file data for v4 (spec v4, LanguageModelV4) models', async () => {
      const list = new MessageList({ threadId, resourceId });
      list.add('Take a screenshot', 'input');
      const continuationMessage: AIV5ModelMessage = {
        role: 'assistant',
      ...
      list.add(continuationMessage, 'input');
      // Genuine spec-v4 (LanguageModelV4) providers like Bedrock removed
      // image-data/file-data entirely and only accept the tagged file shape.
      const prompt = await list.get.all.aiV7.llmPrompt();
      const toolRole = prompt.find(m => m.role === 'tool');
      const toolResultPart = (toolRole as any).content.find((p: any) => p.type === 'tool-result');
      expect(toolResultPart.output).toEqual({
        type: 'content',
        value: [{ type: 'file', data: { type: 'data', data: 'base64imagedata' }, mediaType: 'image/jpeg' }],
      });
    });

    it('translates non-image tool-result media to tagged file data for v4 (spec v4, LanguageModelV4) models', async () => {
      const list = new MessageList({ threadId, resourceId });
      list.add('Fetch a document', 'input');
      const continuationMessage: AIV5ModelMessage = {
        role: 'assistant',
